...
import { getSessionSnapshot } from '@/api/conversations'
...
export function ChatDetailPage() {
  ...
  // Refs to avoid stale closure issues in useCallback
  ...
  useEffect(() => {
    ...
    getSessionSnapshot(conversationId)
      ...
      .catch(() => {
        // No snapshot yet - new conversation
      })
  ...
  const handleSend = useCallback(async () => {
    ...
    // Guard: prevent double-send while already streaming
    if (!currentInput.trim() || !conversationId || !token || streamingRef.current) return
    ...
    try {
      const url = `/api/conversations/${conversationId}/stream?message=${encodeURIComponent(currentInput)}`
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-7rem)] max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 py-3 border-b mb-4">
        ...
        <h2 className="font-semibold flex-1">对话</h2>
      </div>
    ...
    return (
      <div className="flex gap-2 items-start">
        <div className="bg-accent rounded-lg px-4 py-2 text-sm border border-accent-foreground/10 max-w-[80%]">
          <p className="font-medium text-xs text-muted-foreground">🤖 子 Agent: {message.agentName}</p>
      ...
      <div className="bg-muted rounded-lg px-4 py-2 text-sm max-w-[80%]">
        {message.reasoning && (
          <details className="mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1 whitespace-pre-wrap">{message.reasoning}</p>
          </details>
        )}
  ...
  switch (event.event) {
    ...
    case 'reasoning_delta':
      setMessages((prev) => {
        const last = prev[prev.length - 1]
        if (last?.role === 'assistant') {
          return [...prev.slice(0, -1), { ...last, reasoning: (last.reasoning ?? '') + event.data.content }]
        }
        return prev
      })
      break
    case 'tool_call_start':
      setMessages((prev) => {
        const last = prev[prev.length - 1]
        if (last?.role === 'assistant') {
          const tc = { id: event.data.id, name: event.data.name, arguments: event.data.arguments }
    ...
    case 'agent_call_end':
      ...
      break
    // Terminal events: explicitly stop streaming so the send button
    // recovers even if the HTTP stream has not fully closed yet.
    case 'turn_complete':
    case 'done':
      setStreaming(false)
      break
