            ...
            <div key={log.id} className="rounded-md border p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground">
                  ...
                  {log.finished_at &&
                    ` · 耗时 ${((new Date(log.finished_at).getTime() - new Date(log.started_at).getTime()) / 1000).toFixed(1)}s`}
                </span>
              </div>
              {log.output && (
                <details>
                  <summary className="text-sm cursor-pointer">输出</summary>
                  <pre className="text-xs mt-1 whitespace-pre-wrap bg-muted p-2 rounded max-h-48 overflow-y-auto">
                    {log.output}
                  </pre>
                </details>
              )}
