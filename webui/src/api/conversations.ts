...
export async function updateConversation(
  ...
  const res = await api.patch<Conversation>(`${chatBase(agentId)}/${convId}`, data)
  return res.data
}
...
export async function deleteConversation(
  ...
  const res = await api.delete<SuccessResponse>(`${chatBase(agentId)}/${convId}`)
  return res.data
}
...
export async function getMessages(
  ...
  const res = await api.get<Message[]>(`${chatBase(agentId)}/${convId}/messages`, {
    params: { offset, limit },
  })
  return res.data
}
