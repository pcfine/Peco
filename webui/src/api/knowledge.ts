...
import type { CreateKbRequest, Document, KnowledgeBase, SyncResult } from '@/types/knowledge'
...
export async function deleteKnowledgeBase(id: string): Promise<SuccessResponse> {
...
export async function listDocuments(
  ...
  const res = await api.get<Document[]>(`/knowledge/${kbId}/documents`, { params })
  return res.data
}
...
export async function deleteDocument(kbId: string, docId: string): Promise<SuccessResponse> {
  const res = await api.delete<SuccessResponse>(`/knowledge/${kbId}/documents/${docId}`)
  return res.data
}
